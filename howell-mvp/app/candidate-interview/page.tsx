  ...
  async function startInterview() {
    ...
    if (simliEnabled && simliConfigRef.current && simliVideoRef.current && simliAudioRef.current) {
      ...
      try {
        ...
        // Register event listeners BEFORE Initialize (v1.x event emitter API)
        ...
        // Create session token — required for custom face IDs
        let sessionToken = ''
        try {
          const tokenRes = await client.createSessionToken('', {
          ...
          console.warn('[Simli] Could not get session token, proceeding without:', e)
        }
